        ...
        <div className="bg-dark-bg border border-dark-border rounded-lg p-3 hover:border-slate-600 transition-colors">
           <div className="flex justify-between items-start mb-2">
              <div className="flex items-center gap-2">
                 <span className="font-bold text-sm text-slate-200">{comment.authorName}</span>
...
const Forum: React.FC = () => {
    ...
    const fetchThreads = async () => {
        ...
        try {
            if (activeCategory === 'all') {
                 // Mock behavior for 'all': Query a default category for demo purposes
  ...
  return (
    <div className="h-full flex bg-dark-bg overflow-hidden">
        {/* LEFT SIDEBAR - Categories & Private Chat (Desktop) */}
        <div className="w-64 bg-dark-surface border-r border-dark-border flex-col hidden md:flex">
            ...
            {/* Private Chats List - Placeholder for now */}
        ...
        <div className="flex-1 flex flex-col min-w-0">
            ...
            {/* Header */}
            <div className="h-16 border-b border-dark-border bg-dark-surface/80 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-20">
                {!activeThread ? (
                    <>
                        <div className="flex items-center gap-4">
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="bg-dark-bg border border-dark-border rounded-full pl-10 pr-4 py-1.5 text-sm text-white focus:border-brand-500 w-64 transition-all focus:w-80 outline-none"
                                />
                            </div>
                        ...
                        <button 
                            ...
                            className="bg-brand-600 hover:bg-brand-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-medium text-sm shadow-lg shadow-brand-900/20 transition-all"
                        >
                    ...
                    <div className="flex items-center gap-4 w-full">
                        ...
                        <span className="text-sm text-slate-400">
                            <span className="text-slate-500">Category:</span> {categories.find(c => c.id === activeThread.categoryId)?.name}
                        </span>
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
                ...
                {!activeThread ? (
                    <div className="grid grid-cols-1 gap-4 max-w-5xl mx-auto">
                        ...
                        {filteredThreads.map(thread => (
                            <div 
                                ...
                                className="bg-dark-surface border border-dark-border rounded-xl p-5 cursor-pointer hover:border-brand-500/50 hover:shadow-lg hover:shadow-brand-900/10 transition-all group"
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-2">
                                        <img src={thread.authorAvatar} className="w-6 h-6 rounded-full" />
                                        <span className="text-sm font-bold text-slate-300 group-hover:text-brand-400 transition-colors">{thread.authorName}</span>
                                        <span className="text-xs text-slate-500">• {timeAgo(thread.createdAt)}</span>
                                    </div>
                                ...
                                <h3 className="text-lg font-bold text-slate-100 mb-2 group-hover:text-white transition-colors">{thread.title}</h3>
                                <p className="text-slate-400 text-sm line-clamp-2 mb-4">{thread.content}</p>
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-dark-border/50">
                                    <div className="flex items-center gap-4 text-xs text-slate-500 font-medium">
                                        ...
                                        <span className="flex items-center gap-1">
                                            <ThumbsUp className="w-3 h-3" /> {thread.likes} Likes
                                        </span>
                                    </div>
                                    <div className="text-xs text-slate-600 flex items-center gap-1">
                                        Active {timeAgo(thread.lastActive)}
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
                        ...
                        {/* Original Post */}
                        <div className="bg-dark-surface border border-dark-border rounded-xl p-6 mb-6 shadow-xl">
                             <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full p-0.5 bg-gradient-to-br from-brand-400 to-purple-500">
                                        <img src={activeThread.authorAvatar} className="w-full h-full rounded-full border-2 border-dark-surface" />
                                    </div>
                                    <div>
                                        <h1 className="text-2xl font-bold text-white leading-tight">{activeThread.title}</h1>
                                        <div className="flex items-center gap-2 text-sm mt-1">
                                            <span className="text-brand-400 font-medium">{activeThread.authorName}</span>
                                            <span className="text-slate-500">• {timeAgo(activeThread.createdAt)}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                     <button className="p-2 text-slate-400 hover:text-white hover:bg-dark-bg rounded transition-colors"><Send className="w-5 h-5" /></button>
                             ...
                             <div className="prose prose-invert max-w-none text-slate-300 mb-8 text-base leading-relaxed">
                        ...
                        {/* Comments Section */}
                        <div className="relative">
                             <div className="absolute left-8 top-0 bottom-0 w-px bg-dark-border -z-10"></div>
